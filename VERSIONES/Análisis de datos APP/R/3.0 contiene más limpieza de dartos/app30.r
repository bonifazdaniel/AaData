...
server <- function(input, output, session) {
    ...
    if (rv$paso == 1) {
      ...
      div(class = "caja",
        ...
        fluidRow(
          ...
          column(6,
            h4("Opciones avanzadas de limpieza"),
        ...
        # Paneles condicionales contextuales según las casillas seleccionadas
        conditionalPanel(
      ...
      div(class = "caja",
        ...
        radioButtons("tipo_analisis", NULL,
          choices = c(
            ...
            "Evolución en el tiempo" = "longitudinal"
          )),
        hr(),
  ...
  ## =====================================================================
  ## PASO 4: Limpieza (Ampliación con las 6 opciones solicitadas)
  ## =====================================================================
  ...
  observeEvent(input$aplicar_limpieza, {
    ...
    # --- 1. Remoción de Espacios Original ---
    ...
    # --- 2. [Nueva] Limpiar nombres de columnas ---
    if ("limpiar_nombres" %in% input$opciones_limpieza_nuevas) {
      ...
      acciones <- c(acciones, "Nombres de columnas estandarizados (sin acentos ni caracteres especiales)")
    }

    # --- 3. [Nueva] Convertir texto a números ---
    ...
    # --- 4. [Nueva] Estandarizar texto en variables categóricas ---
    if ("estandarizar_texto" %in% input$opciones_limpieza_nuevas) {
      ...
      acciones <- c(acciones, "Variables de texto convertidas completamente a MAYÚSCULAS")
    }

    # --- 5. [Nueva] Filtrar valores fuera de un rango válido ---
    ...
    # --- 6. [Nueva] Detectar y tratar valores atípicos (Outliers) ---
    if ("tratar_outliers" %in% input$opciones_limpieza_nuevas && !is.null(input$cols_outliers)) {
      ...
      acciones <- c(acciones, sprintf("Detectados %d outliers en columnas numéricas (Tratamiento: %s)", total_outliers, input$metodo_outliers))
    }

    # --- 7. [Nueva] Redondear valores numéricos ---
    if ("redondear_num" %in% input$opciones_limpieza_nuevas && !is.null(input$cols_a_redondear)) {
      ...
      acciones <- c(acciones, paste("Columnas redondeadas a", dec, "decimales:", paste(cols_red, collapse = ", ")))
    }

    # --- 8. Remoción de NAs, Duplicados y Columnas Vacías Originales ---
    ...
    if ("quitar_na" %in% input$opciones_limpieza) {
      ...
      acciones <- c(acciones, sprintf("Se eliminaron %d fila(s) con celdas vacías (NA)", n_antes - nrow(df)))
    }
    ...
    if (length(acciones) == 0) {
      agregar_bitacora("Se ejecutó el módulo de limpieza sin seleccionar cambios.")
    } else {
      agregar_bitacora(paste0("Limpieza ejecutada con éxito: ", paste(acciones, collapse = "; "), "."))
    }
    ...
    output$resultado_limpieza <- renderUI({
      div(style = "margin-top: 15px; padding: 12px; background-color: #eef7ee; border-radius: 6px;",
          ...
          p(sprintf("Dimensiones Iniciales: %d filas x %d columnas.", filas_antes, cols_antes)),
          p(sprintf("Dimensiones Finales Post-Limpieza: %d filas x %d columnas.", nrow(df), ncol(df))),
  ...
  observeEvent(input$run_comp, {
    ...
    if (n_grupos < 2) {
      resultado_texto <- "Faltan grupos suficientes para realizar contrastes estadísticos."
    } else {
      ...
      if (n_grupos == 2) {
        ...
        resultado_texto <- paste0("Comparación de 2 grupos con ", if(es_normal)"T-Student"else"Wilcoxon", ".\n\n", interpretar_p(prueba$p.value))
      } else {
        if (es_normal) {
        ...
        resultado_texto <- paste0("Comparación múltiple de grupos.\n\n", interpretar_p(p_valor))
      }
    }
    output$out_comp <- renderText(resultado_texto)
    output$out_comp_tabla <- renderTable(tabla_resultado)
    rv$resultados$comparacion <- list(titulo = sprintf("Comparación de: %s", input$comp_num), texto = resultado_texto, tabla = tabla_resultado)
    agregar_bitacora(sprintf("Test de comparación ejecutado en '%s'.", input$comp_num))
  })
  ...
  ## =====================================================================
  ## PASO 7: Descarga de Resultados y PDF Report Builder
  ## =====================================================================
  ...
  pagina_tabla_pdf <- function(titulo, df) {
    ...
    tabla_grob <- gridExtra::tableGrob(df, rows = NULL, theme = gridExtra::ttheme_default(base_size = 9))
    gridExtra::grid.arrange(tabla_grob, top = grid::textGrob(titulo, gp = grid::gpar(fontsize = 14, fontface = "bold", col = "#7a1f3d")))
  }
