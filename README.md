...
Aplicación que guía al usuario, pregunta tras pregunta, para:
...
En los pasos de limpieza, análisis y gráficas hay un **Asistente inteligente**:
escribes para qué usarás tus datos y te recomienda qué limpieza, qué análisis
y qué gráfica convienen, según el tamaño de tu muestra y tu objetivo. Por
defecto todo se calcula localmente (no se envían tus datos). Opcionalmente se
puede activar una "IA avanzada" que usa la API de Anthropic enviando solo un
resumen de las variables (nunca los datos crudos de pacientes) y que requiere
tu propia clave de API.
...
`Grupo`, `Dia`, `Puntaje\_clinico`, `Peso\_g`, `IL6\_pg\_mL`) y cada **fila es
...
* `Puntaje\_clinico`: puntaje de severidad de artritis.
* `Peso\_g`: peso corporal en gramos.
* `IL6\_pg\_mL`: nivel de la citocina IL-6 en plasma.
* `Dia\_evento\_artritis`: día (simulado) en que el ratón desarrolla artritis
clínica manifiesta (o 28 si no la desarrolló).
* `Evento\_artritis`: 1 si el evento ocurrió, 0 si el ratón quedó censurado.
